        ...
        book_renter_response["response"]["book"]["prices"].each do |item|
          if item["term"] # rentals
            results << {:vendor => "Book Renter",
                        ...
                        :buy_link => "",
                        :condition => "Rental",
                        :rent_link => buy_url + Rack::Utils.escape(cart_url.sub('RENTAL_PERIOD', item["days"].to_s)),
                        :shipping => 0,
            ...
            if item["condition"] # purchase
              results << {:vendor => "Book Renter",
                          ...
                          :buy => true,
                          :rent => false,
                          :cart_link => buy_url + Rack::Utils.escape(cart_url.sub('RENTAL_PERIOD', 90.to_s)),
                          :buy_link => buy_url + Rack::Utils.escape(cart_url.sub('RENTAL_PERIOD', 90.to_s)),
